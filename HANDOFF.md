...
## 지금 진행 중이던 것 (P2-12까지 완료, P2-13부터 이어갈 것)

P2-4~P2-12가 전부 완료되어 GitHub main에 커밋+푸시됨(최신 커밋은 `git log`로 확인). 각 단계의
...
**다음은 P2-13(Claude Code 프로젝트 권한 설정)** — `.claude/settings.json` 프로젝트 전용 파일
신설, company-manager MCP 도구별 정확한 allowlist(`mcp__company-manager__도구이름` 패턴).
`--dangerously-skip-permissions` 사용 금지, Bash 전체/외부쓰기/영구삭제 등은 절대 allow 안 함.

## 남은 전체 로드맵 (P2-13 ~ P2-14)
