...
//
// `animated` is a separate concern from `icon` (P2-11): it drives a
// low-intensity breathing ring meant for states where the employee is
// actively being worked on right now (running, reviewing) — everything
// else (waiting, blocked, terminal) stays still, and respects
// prefers-reduced-motion regardless of state.
const STATUS_STYLE: Record<
  EmployeeStatus,
  { bg: string; icon: "dot" | "clock" | "doc" | "exclaim" | "x" | "pause" | "check"; animated?: boolean }
> = {
  idle: { bg: "#9AA4AC", icon: "dot" },
  queued: { bg: "#E6B85C", icon: "clock" },
  running: { bg: "#5FB97A", icon: "dot", animated: true },
  awaiting_approval: { bg: "#5F9BD9", icon: "doc" },
  needs_review: { bg: "#E68A3B", icon: "exclaim" },
  reviewing: { bg: "#8E7CC3", icon: "doc", animated: true },
...
export function StatusBadge({ status }: { status: EmployeeStatus }) {
  ...
  const cx = 80;
  const cy = 16;
  const showRing = style.animated && !reducedMotion;
